...
async function main() {
    ...
    try {
        ...
        assert.match(await admin.locator('#adminContent').textContent(), /Актуальных домашних заданий пока нет/);
        ...
        await admin.getByRole('button', {name:'Добавить задание'}).click();
        assert.equal(await admin.locator('.admin-record').filter({hasText:'Прошедшее тестовое задание'}).count(), 0);
        await admin.locator('#adminContent').getByRole('button', {name:/^Архив ·/}).click();
        ...
        const archivedHomeworkRecord = admin.locator('.admin-record').filter({hasText:'Прошедшее тестовое задание'});
        await archivedHomeworkRecord.locator('summary').click();
        admin.once('dialog', dialog => dialog.accept());
        await archivedHomeworkRecord.getByRole('button', {name:'Удалить из архива'}).click();
        await archivedHomeworkRecord.waitFor({state:'detached'});
        checks.push('Homework manager separates current work from a compact, directly deletable archive');
        ...
        await createdTopic.getByRole('button', {name:'Выбрать тему'}).click();
        assert.match(await b.getByRole('button', {name:/Мои доклады/}).textContent(), /· 1/);
        await createdTopicRecord.getByRole('button', {name:'В архив'}).click();
        await b.evaluate(() => refreshState());
        assert.match(await b.getByRole('button', {name:/Мои доклады/}).textContent(), /· 0/);
        assert.equal(await b.locator('#topicsContainer .topic-card').filter({hasText:'Тестовая тема администратора'}).count(), 0);
        await admin.locator('#adminContent').getByRole('button', {name:/^Архив ·/}).click();
        const archivedBookedTopic = admin.locator('.admin-record').filter({hasText:'Тестовая тема администратора'});
        await archivedBookedTopic.waitFor();
        await archivedBookedTopic.locator('summary').click();
        admin.once('dialog', dialog => dialog.accept());
        await archivedBookedTopic.getByRole('button', {name:'Удалить из архива'}).click();
        await archivedBookedTopic.waitFor({state:'detached'});
        checks.push('Admin archives are separated; archived reports delete in one step and My reports count stays current');
        await admin.locator('#adminContent').getByRole('button', {name:/^Актуальные ·/}).click();
        ...
        await admin.getByRole('button', {name:'Добавить тему'}).click();
        assert.equal(await admin.locator('.admin-record').filter({hasText:'Архивный тестовый доклад'}).count(), 0);
        await admin.locator('#adminContent').getByRole('button', {name:/^Архив ·/}).click();
