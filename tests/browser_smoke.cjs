...
async function main() {
    ...
    try {
        ...
        assert.match(await homeworkDownload.getAttribute('href'), /\/files\/[a-f0-9]+\.pdf\?name=.*download=1/);
        const materialDownloadPromise = b.waitForEvent('download');
        await homeworkDownload.click();
        const materialDownload = await materialDownloadPromise;
        assert.equal(materialDownload.suggestedFilename(), 'Задание.pdf');
        const materialViewer = await b.context().newPage();
        materialViewer.on('pageerror', error => errors.push(error.message));
        await materialViewer.goto(homeworkPreviewUrl, {waitUntil:'domcontentloaded'});
        assert.equal((await materialViewer.locator('#viewerTitle').textContent()).trim(), 'Задание.pdf');
        assert.match(await materialViewer.locator('.viewer-frame').getAttribute('src'), /\/files\/[a-f0-9]+\.pdf/);
        const officeFileUrl = `${base}/files/example.docx?name=${encodeURIComponent('Материалы.docx')}`;
        await materialViewer.goto(`${base}/viewer.html?file=${encodeURIComponent(officeFileUrl)}`, {waitUntil:'domcontentloaded'});
        assert.match(await materialViewer.locator('.viewer-frame').getAttribute('src'), /^https:\/\/view\.officeapps\.live\.com\/op\/embed\.aspx\?src=/);
        await materialViewer.close();
        assert.equal(await homework.getByRole('button').count(), 0);
        checks.push('Homework is read-only for students; files download and PDF or Office previews open on the site');
        await admin.locator('#newAssignmentSubject').selectOption({label:'Управление бизнес-процессами'});
